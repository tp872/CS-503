...
//Prototypes inform the compiler about the function's names,
//return types, and parameter lists before their actual 
//definitions appear in the code. This ensures that the 
//compiler can verify function calls for correctness, such 
//as the number and type of arguments passed, even if the 
//function implementation is defined later.
void  usage(char *);
...
int main(int argc, char *argv[]){
    ...
    switch (opt){
        ...
        case 'r':
            ...
            //The string is reversed in place because the function 
            //manipulates the characters of the original string 
            //directly in memory. It swaps the characters within 
            //the same memory buffer without creating a copy or 
            //additional storage. This is efficient and preserves 
            //the original pointer to the string.
        ...
        //          your thoughts.
        //The default option serves as a catch-all for invalid or 
        //unsupported options provided by the user. Its purpose is 
        //to ensure that the program handles unexpected inputs 
    ...
    //A break statement ensures that the program exits the 
    //current case in the switch statement after executing 
    //its logic. Without the break, the program would 
    //"fall through," executing the subsequent cases 
    //regardless of whether their conditions match. 
    //This could lead to incorrect results or unintended behavior.
    //For the default case, a break is unnecessary because the 
    //program either exits (exit(1)) or logically terminates the switch block.
    return 0;
}