...
int exec_remote_cmd_loop(char *address, int port) {
    ...
    char *tx_buffer = malloc(page_size);
    char *rx_buffer = malloc(page_size);
    
    if (!tx_buffer || !rx_buffer) {
        return client_cleanup(-1, tx_buffer, rx_buffer, ERR_MEMORY);
    }

    int network_socket = start_client(address, port);
    if (network_socket < 0) {
        return client_cleanup(network_socket, tx_buffer, rx_buffer, ERR_RDSH_CLIENT);
    }
    ...
    while (1) {
        ...
        if (!fgets(tx_buffer, page_size, stdin)) {
            break;
        }

        size_t cmd_length = strlen(tx_buffer);
        while (cmd_length > 0 && (tx_buffer[cmd_length-1] == '\n' || tx_buffer[cmd_length-1] == '\r')) {
            tx_buffer[--cmd_length] = '\0';
        }

        size_t remaining = cmd_length + 1;
        size_t offset = 0;
        while (remaining > 0) {
            ssize_t sent = send(network_socket, tx_buffer + offset, remaining, MSG_MORE);
            if (sent <= 0) {
                return client_cleanup(network_socket, tx_buffer, rx_buffer, ERR_RDSH_COMMUNICATION);
            }
            remaining -= sent;
            offset += sent;
        }
        ...
        while (1) {
            ...
            int ready = select(network_socket + 1, &read_fds, NULL, NULL, &timeout);
            if (ready <= 0) {
                return client_cleanup(network_socket, tx_buffer, rx_buffer, ERR_RDSH_COMMUNICATION);
            }

            ssize_t received = recv(network_socket, rx_buffer, page_size - 1, 0);
            if (received <= 0) {
                return client_cleanup(network_socket, tx_buffer, rx_buffer, ERR_RDSH_COMMUNICATION);
            }

            write(STDOUT_FILENO, rx_buffer, received);

            if (rx_buffer[received - 1] == RDSH_EOF_CHAR) {
                break;
            }
        }
    }

    return client_cleanup(network_socket, tx_buffer, rx_buffer, OK);
}
...
int start_client(char *server_ip, int port) {
    ...
    if (client_sock < 0) return ERR_RDSH_CLIENT;
    
    setsockopt(client_sock, SOL_SOCKET, SO_KEEPALIVE, &sock_opts, sizeof(sock_opts));
